...
const CorrectionModal: React.FC<CorrectionModalProps> = ({ isOpen, onClose, record }) => {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 fade-in duration-200">
        ...
        <div className="p-8 space-y-8">
          <div className="flex items-center gap-6">
            <label className="w-24 text-sm font-semibold text-slate-600 text-right shrink-0">
              <span className="text-red-500 mr-1">*</span>补签到:
            </label>
            <div className="flex-1 relative">
              <input 
                className="w-full pl-4 pr-10 py-3 bg-slate-50 border-slate-200 rounded-xl text-sm focus:ring-primary focus:border-primary transition-all shadow-inner" 
                type="text" 
                defaultValue={record?.correctionTime || '2026/01/14 09:00'}
              />
              <span className="absolute right-3.5 top-1/2 -translate-y-1/2 material-symbols-outlined text-slate-400">calendar_today</span>
            </div>
          </div>
        ...
        <div className="px-8 py-6 bg-slate-50/50 border-t border-slate-100 flex justify-center gap-4">
          <button 
            onClick={onClose}
            className="px-12 py-3 bg-primary hover:bg-primary-hover text-white font-bold rounded-xl shadow-lg shadow-primary/25 active:scale-95 transition-all text-sm"
          >
            确认
          </button>
          <button 
