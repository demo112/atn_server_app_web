...
// Fix imports to point to jest-expo package
const jestExpoRoot = path.dirname(require.resolve('jest-expo/package.json'));
const publicExpoModules = require(path.join(jestExpoRoot, 'src/preset/moduleMocks/expoModules'));
const internalExpoModules = require(path.join(jestExpoRoot, 'src/preset/moduleMocks/internalExpoModules'));
const thirdPartyModules = require(path.join(jestExpoRoot, 'src/preset/moduleMocks/thirdPartyModules'));
...
if (expoModules.NativeUnimoduleProxy && expoModules.NativeUnimoduleProxy.modulesConstants && expoModules.NativeUnimoduleProxy.modulesConstants.mockDefinition && expoModules.NativeUnimoduleProxy.modulesConstants.mockDefinition.ExponentConstants) {
    expoModules.NativeUnimoduleProxy.modulesConstants.mockDefinition.ExponentConstants.experienceUrl.mock = 'exp://192.168.1.200:8081';
}
...
// PATCHED: Check if UIManager exists before defining property
if (mockNativeModules.UIManager && mockNativeModules.NativeUnimoduleProxy && mockNativeModules.NativeUnimoduleProxy.viewManagersMetadata) {
...
jest.doMock('expo-modules-core', () => {
  ...
  // PATCHED: Check if NativeModulesProxy exists
