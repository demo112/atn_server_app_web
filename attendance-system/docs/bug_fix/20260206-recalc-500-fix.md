...
- **修复思路**：
  1. 后端：
     - 在 `AttendanceScheduler.init` 中增加 Redis 版本检查（需 >= 5.0.0），版本过低时自动禁用调度器。
     - 增加 Redis 连接异常处理，防止初始化失败导致应用崩溃。
     - 在 `triggerCalculation` 中检查 `queue` 是否存在，若不存在抛出 `AppError` (503 Service Unavailable)。
