...
## 根因分析
- **直接原因**：后端返回 `null`，Zod Schema 期望 `undefined`。
- **根本原因**：Schema 定义未考虑到 JSON 序列化中 `null` 的存在（后端通常将 `optional` 字段序列化为 `null` 而非省略）。
- **相关代码**：`packages/web/src/schemas/attendance.ts` 中的 `TimePeriodSchema`。

## 修复方案
- **修复思路**：修改 Zod Schema，允许字段为 `nullable`，并在转换（transform）阶段将 `null` 转换为 `undefined`，以保持与前端类型的兼容性。
- **改动文件**：
...
- [x] 设计一致性确认：修复后的类型 `string | undefined` 符合 `design.md` 和 Shared Type 定义。
...
背景: 后端返回的班次时间段数据中，可选字段值为 null
变更: 更新 TimePeriodSchema，支持 nullable 并转换为 undefined
影响: 班次管理列表页数据加载
