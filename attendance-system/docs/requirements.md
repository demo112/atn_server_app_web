...
1. **用户 (User)**:
   - 定义：登录系统的账号主体，拥有用户名和密码。
   - 职责：用于身份认证和系统操作权限控制。
   - 关系：一个用户必须关联一个人员（实名制），但一个人员不一定有对应的用户账号（如仅需打卡的普通工人）。

2. **人员 (Employee)**:
   - 定义：企业内的自然人，考勤管理的最小单位。
   - 职责：进行打卡、排班、请假、统计考勤数据的主体。
   - 特性：
     - 可以手动录入和修改信息。
     - 是所有考勤数据（打卡、日报、月报）的关联对象。
     - 人员信息包含姓名、工号、部门、手机号等业务属性。
...
### 人A（sasuke）：用户/组织/统计模块

| 模块 | 说明 |
|------|------|
| 用户管理 | 用户账号CRUD、认证、权限 |
| 人员管理 | 人员档案管理（入职、离职、信息维护） |
| 组织架构 | 部门树管理 |
| 考勤统计 | SW70、SW71、SW72 |
