...
### 7. Zod 运行时校验集成 (L1/L3)
- **Governance (L1)**: 构建了 User, Auth, Department 等核心业务的 Zod Schema 防腐层。
- **Operations (L3)**: 在 Web 和 App 端的 Service 层全面集成运行时校验，拦截非法数据。
- **Refactor**: 移除了 App 端 Screen 层冗余的 `.success`/`.data` 检查，统一由 Service 层返回纯净数据。
