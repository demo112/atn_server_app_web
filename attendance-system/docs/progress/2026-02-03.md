...
### 6. UI 仿制 (班次管理)
- **Web**: 重构 `ShiftPage` 并新增 `ShiftModal` 组件，精准还原 `shift_list.html` 和 `shift_add.html` 视觉设计
- **Features**: 班次列表展示、新增/编辑班次弹窗（含多时段配置、打卡规则）

### 7. App 端属性测试 (QA-APP-PROP)
- **工具**: 引入 `fast-check` 进行 Property-based Testing。
- **Utils 层**:
  - `auth.ts`: 验证 `setToken`/`getToken` 和 `setUser`/`getUser` 的往返一致性 (Round-trip)。发现并修复了 `getToken` 在空字符串时的处理缺陷 (返回 `null` 而非 `""`)。
  - `error.ts`: 验证 `getErrorMessage` 对各种异常输入（Error对象、Axios错误、任意对象）的鲁棒性。增强了对空消息的处理（回退到默认消息）。
- **Schemas 层**:
  - 覆盖所有核心 Zod Schemas (`auth`, `user`, `employee`, `department`, `attendance`)。
  - 验证了 `safeParse` 对随机生成数据的鲁棒性，确保不会抛出未捕获异常。
  - 修正了 Schema 定义与实际数据可能存在的 `null` vs `undefined` 不一致问题。
- **成效**: 
  - 修复 `getToken` 空值处理 Bug。
  - 增强 `getErrorMessage` 健壮性。
  - 验证了 5 个模块的 20+ 个 Schema 的数据验证逻辑。
  - 属性测试套件 (7 suites) 全部通过。
